...
export default function Shop() {
  ...
  const setCategory = (key) => { setFilter(key); key === 'all' ? setSearchParams({}) : setSearchParams({ cat: key }) }
  const filtered = filter === 'all' ? PRODUCTS.filter(p => p.available) : PRODUCTS.filter(p => p.category === filter && p.available)
  return (
    <>
      <Toast />
      <div style={{ paddingTop: 'calc(var(--nav-h) + 60px)', paddingBottom: 60, background: 'var(--cream)' }}>
        <div className="container">
          ...
          <div className="shop-filters reveal">
            {FILTERS.map(f => <button key={f.key} className={`filter-btn${filter === f.key ? ' active' : ''}`} onClick={() => setCategory(f.key)}>{f.label}</button>)}
            <span className="shop-sort">{filtered.length} objects</span>
          </div>
          <div className="shop-grid">
            {filtered.map((p, i) => <ProductCard key={p.slug} product={p} delayClass={`d${(i % 3) + 1}`} />)}
          </div>
        </div>
      </div>
      <Newsletter />
