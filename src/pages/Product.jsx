        ...
        <div className="product-detail__inner">
          ...
          <div className="product-info">
            ...
            <div className="product-info__price-row">
              <span className="product-info__price">{product.slug === 'duo-statue' ? 'From ' : ''}EGP {product.price.toLocaleString()}</span>
            </div>
            <p className="product-info__desc">{product.description}</p>
            <div style={{ marginBottom: 28 }}>
              <p className="product-info__option-label">Colour — <span style={{ color:'var(--bronze)',textTransform:'capitalize' }}>{activeColour.name}</span></p>
              <div className="product-info__colors">
                {PRODUCT_COLOURS.map(c => (
                  <div key={c.value} className={`color-swatch color-swatch--${c.value}${activeColour.value === c.value ? ' active' : ''}`}
                    style={{ background: c.hex }} onClick={() => setActiveColour(c)} title={c.name} />
                ))}
              </div>
            </div>
            {product.customisable && (
              <div className="product-info__custom" style={{ marginBottom: 28 }}>
                ...
                <textarea className="product-info__textarea" placeholder="Any custom details, names, or requests..."
            ...
            <div style={{ display:'flex', gap:12, marginBottom:12 }}>
              ...
              <button className="btn btn-bronze" style={{ flex:1 }}
                onClick={() => addToCart(slug, qty, { variantName: activeColour.name, personalisationNote: note || null })}>
                Add to cart
              </button>
            </div>
        ...
        {related.length > 0 && (
          <div style={{ padding:'80px 0', background:'var(--light)' }}>
            <div className="container">
              <div className="section-header reveal" style={{ marginBottom:40 }}><h2 className="t-h2">You might also like</h2></div>
              <div className="grid-3 reveal d1">{related.map(p => <ProductCard key={p.slug} product={p} />)}</div>
            </div>
          </div>
        )}
      </div>
