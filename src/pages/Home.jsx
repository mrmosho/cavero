...
export default function Home() {
  ...
  return (
    <>
      ...
      <section className="section" style={{ background:'var(--cream)' }}>
        <div className="container">
          <div className="section-header reveal">
            <div><p className="t-label section-header__eyebrow">The collection</p><h2 className="t-h1">Made for your space.</h2></div>
            <Link to="/shop" className="section-header__link">View all objects</Link>
          </div>
          <div className="grid-3" style={{ gap:'40px 28px' }}>
            {PRODUCTS.slice(0,6).map((p,i) => <ProductCard key={p.slug} product={p} delayClass={`d${(i%3)+1}`}/>)}
          </div>
        ...
        <div className="container">
          ...
          <div className="grid-2 reveal d1" style={{ gap:16 }}>
            ...
            <div style={{ display:'grid', gridTemplateRows:'1fr 1fr', gap:16 }}>
              {[['For the home','Vases \u00b7 Planters \u00b7 Wall art','linear-gradient(135deg,#4A4452,#3D3844)'],['For the desk','Stands \u00b7 Holders \u00b7 Trays','linear-gradient(135deg,#2D2B34,#3D3844)']].map(([title,sub,bg]) => (
                <Link key={title} to="/gifting" className="gift-card">
                  <div className="gift-card__placeholder" style={{ background:bg }}><svg viewBox="0 0 80 80" width="50" height="50" fill="none" stroke="rgba(168,149,111,0.5)" strokeWidth="1.5"><path d="M20 10 Q40 5 60 10 Q70 55 60 70 Q50 78 40 78 Q30 78 20 70 Q10 55 20 10Z"/></svg></div>
                  <div className="gift-card__overlay"/>
      ...
      <section className="process">
        <div className="container">
          ...
          <div className="process__steps reveal d1">
            {[['01','You choose','Browse the collection. Pick your piece, colour, and any personalisation details.'],['02','We craft','Your piece enters production in our Cairo studio. Designed, shaped, and finished by hand.'],['03','We pack','Wrapped in cream tissue, sealed with a Cavero card. Packed like it matters \u2014 because it does.'],['04',"It's yours",'Delivered to your door in 5\u20137 days. Made specifically for you. No two pieces exactly alike.']].map(([num,title,desc]) => (
      ...
      <section className="section" style={{ background:'var(--cream)' }}>
        <div className="container">
          <div className="section-header reveal"><div><p className="t-label section-header__eyebrow">What people say</p><h2 className="t-h2">Real orders. Real reactions.</h2></div></div>
          <div className="grid-3 reveal d1">
            {REVIEWS.map((r,i) => (
              <div className="review-card" key={i}><div className="review-stars">{'\u2605'.repeat(r.stars)}</div><p className="review-text">{r.text}</p><p className="review-author">{r.author} \u00b7 <span className="review-product">{r.product}</span></p></div>
            ))}
          </div>
        </div>
      </section>
      <section style={{ background:'var(--light)', padding:'60px 0' }}>
        <div className="container">
          <div className="text-center mb-32 reveal"><p className="t-label" style={{ marginBottom:8 }}>Follow along</p><a href="https://instagram.com/cavero.studio" target="_blank" rel="noreferrer" style={{ fontFamily:'var(--font-display)', fontSize:'1.6rem', fontWeight:300, color:'var(--charcoal)' }}>@cavero.studio</a></div>
          <div className="insta-grid reveal d1">
            {[['\u25c7','var(--light)'],['\u25cb','linear-gradient(135deg,#DDD9CE,#C8C4BA)'],['\u25b3','linear-gradient(135deg,#C8C4BA,#DDD9CE)'],['\u25c7','var(--light)'],['\u25cb','linear-gradient(135deg,#DDD9CE,#C8C4BA)'],['\u25b3','linear-gradient(135deg,#C8C4BA,#DDD9CE)']].map(([sym,bg],i) => (
          ...
          <div className="text-center mt-32 reveal"><a href="https://instagram.com/cavero.studio" target="_blank" rel="noreferrer" className="btn btn-outline btn-sm btn-pill">Follow @cavero.studio</a></div>
        </div>
      </section>
      <Newsletter/>
      <Footer/>
