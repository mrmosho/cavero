...
export function CartProvider({ children }) {
  const [cart, setCart] = useState(() => {
    try { return JSON.parse(localStorage.getItem('cavero_cart')) || [] }
    catch { return [] }
  })
  ...
  useEffect(() => {
    localStorage.setItem('cavero_cart', JSON.stringify(cart))
  }, [cart])
  ...
  const addToCart = useCallback((slug, qty = 1, options = {}) => {
    // options: { variantName, personalisationNote }
    const product = PRODUCTS.find(p => p.slug === slug)
    if (!product || !product.available) return

    setCart(prev => {
      // Key by slug + variant so different colours are separate line items
