...
export default function Nav({ dark = false }) {
  ...
  return (
    <>
      ...
      {menuOpen && (
        <>
          <div className="overlay" style={{ zIndex: 999 }} onClick={closeMenu} />
          <div className="mobile-menu" style={{ animation: 'menuIn 0.5s var(--ease-out) both' }}>
            <span className="mobile-menu__close" onClick={closeMenu}>×</span>
            {NAV_LINKS.map(({ label, to }) => (
              <Link key={to} to={to} className="mobile-menu__link" onClick={closeMenu}>{label}</Link>
            ))}
            <div style={{ marginTop: 48 }}>
              <Link to="/contact" className="btn btn-outline-cream btn-sm" onClick={closeMenu}>
                Custom Order
              </Link>
            </div>
          </div>
